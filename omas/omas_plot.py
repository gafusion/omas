...
def profiles_fluxes(ods, time_index=0, fig=None, axes=None,
    ...
    :param fig, axes: figure to plot in (a new figure is generated if `fig is None`)

    :param plotting_label: String to add to plot labels can be left empty

    :param show_total_density: bool
        Show total thermal+fast in addition to thermal/fast breakdown if available

    :param plot_zeff: if True, plot zeff below the rotatation
    ...
    if "core_profiles" in ods:
        ...
        # Temp electrons
        axes[0,0].plot(rho_core_prof, prof1d[ods_species[0]]['temperature']/1e3, ls=linestyle, lw=linewidth, color=color_label_dict.setdefault(plotting_label,'r'), label=ods['summary']['ids_properties']['comment'] + " " + plotting_label)  # keV
        ...
        with omas_environment(ods, coordsio={'equilibrium.time_slice.0.profiles_1d.psi': prof1d['grid']['psi']}):
            ...
            axes[3,0].plot(rho_core_prof, rotation, ls=linestyle, lw=linewidth, color=color_label_dict.setdefault(plotting_label,'r'), label=ods['summary']['ids_properties']['comment'] + " " + plotting_label)  # m/s
