...
@add_to__ODS__
def equilibrium_summary_and_current_and_core_profiles(ods, time_index=None, time=None,  fig=None, ggd_points_triangles=None, 
                          ods_species=[-1,1], **kw):
    """
    Plot equilibrium cross section, pressure, current, constraints, and profiles
    ...
    if isinstance(time_index, (list, numpy.ndarray)):
        if len(time) == 1:
            ...
            return ods_time_plot(
                equilibrium_summary_and_current_and_core_profiles, ods, time_index, time, fig=fig, ggd_points_triangles=ggd_points_triangles, ax=axs, ods_species=ods_species, **kw
            )
    ...
    # current
    ax = cached_add_subplot(fig, axs, 2, 4, 4)
    ax.plot(x, eq['profiles_1d']['j_tor'])
    ytitle = 0.9
    ax.set_title('Toroidal Current Density [A/m^2]', y=ytitle, va='top')
    ax.set_xlabel(xName)
    kw.setdefault('color', ax.lines[-1].get_color())
    j_x = psi2psin(eq['constraints.j_tor.:.position.psi'])
    j = eq['constraints.j_tor.:.measured']
    #j_e = eq['constraints.j_tor.:.measured_error_upper']
    #ax.errorbar(j_x, j, yerr=j_e, color='red', marker='', alpha=0.25)
    ax.errorbar(j_x, j, color='red', marker='o', alpha=0.25)
