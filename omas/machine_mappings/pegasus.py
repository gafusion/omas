...
def pegasus_equil_data(ods, pulse, user_argument='loading data from mds+', verbose = False):
    ...
    for k in ods1['magnetics.flux_loop']:
        ...
        try:
            ...
            if dummy_time is None and 'flux.time' in ods[f'magnetics.flux_loop.{k}']:
                dummy_time = ods[f'magnetics.flux_loop.{k}.flux.time']
            ...
            if dummy_time is not None:
                ods[f'magnetics.flux_loop.{k}.flux.data'] = np.zeros_like(dummy_time, dtype=float)
                ods[f'magnetics.flux_loop.{k}.flux.time'] = dummy_time
            else:
                try:
                    ods[f'magnetics.flux_loop.{k}.flux.data'] = np.zeros_like(ods[f'magnetics.flux_loop.{0}.flux.time'])
                    ods[f'magnetics.flux_loop.{k}.flux.time'] = ods[f'magnetics.flux_loop.{0}.flux.time']
                except:
                     ods[f'magnetics.flux_loop.{k}.flux.data'] = np.zeros(int(1e6), dtype=float)
                     ods[f'magnetics.flux_loop.{k}.flux.time'] = np.zeros(int(1e6), dtype=float)

    # handle uncertainties for flux_loop
    for channel in ods1['magnetics.flux_loop']:
        if f'magnetics.flux_loop.{channel}.flux.data' in ods:
            ...
            identifier = ods1[f'magnetics.flux_loop.{channel}.identifier'].upper()
            if 'NFCL' in identifier:
                sigma = 0.01  # NFCLs have 1% uncertainty
            else:
                sigma = 0.005 # CFLs and WFLs have 0.5% uncertainty
            
            ods[f'magnetics.flux_loop.{channel}.flux.data_error_upper'] = sigma*abs(data)
    ...
    for k in ods1['magnetics.b_field_pol_probe']:
        ...
        try:
            ...
            if dummy_time is None and 'field.time' in ods[f'magnetics.b_field_pol_probe.{k}']: 
                dummy_time = ods[f'magnetics.b_field_pol_probe.{0}.field.time']
            ...
            print(f"{identifier} failed: {e}")
            if dummy_time is not None:
                ods[f'magnetics.b_field_pol_probe.{k}.field.data'] = np.zeros_like(dummy_time, dtype=float)
                ods[f'magnetics.b_field_pol_probe.{k}.field.time'] = dummy_time
            else:
                try:
                    ods[f'magnetics.b_field_pol_probe.{k}.field.data'] = np.zeros_like(ods[f'magnetics.b_field_pol_probe.{0}.field.time'], dtype=float)
                    ods[f'magnetics.b_field_pol_probe.{k}.field.time'] = ods[f'magnetics.b_field_pol_probe.{0}.field.time']
                except:
                    ods[f'magnetics.b_field_pol_probe.{k}.field.data'] = np.zeros(int(1e+6), dtype=float)
                    ods[f'magnetics.b_field_pol_probe.{k}.field.time'] = np.zeros(int(1e+6), dtype=float)

    del dummy_time

    # handle uncertainties for b_field_pol_probe
    # Presently all b_field pol probes have 1% uncertainty
    for channel in ods1['magnetics.b_field_pol_probe']:
        if f'magnetics.b_field_pol_probe.{channel}.field.data' in ods:
            ...
            ods[f'magnetics.b_field_pol_probe.{channel}.field.data_error_upper'] = 0.01*abs(data)
    ...
    ods['magnetics.ip.0.data_error_upper'] = 0.02*ods['magnetics.ip.0.data']
    ...
    ods['magnetics.ip.1.data_error_upper'] = 0.02*ods['magnetics.ip.1.data']
    
    # TF Coil Current
    ods["tf.b_field_tor_vacuum_r.time"] = c.get('dim_of(\\TOP.AUTOMAG:iTF)').data()
    ods["tf.b_field_tor_vacuum_r.data"] = 4e-7*np.pi * c.get('\\TOP.AUTOMAG:iTF').data() / (2*pi*0.4) #R0 = ods['tf.r0'] = 0.4 [m]
    ods["tf.b_field_tor_vacuum_r.data_error_upper"] = 0.01*ods["tf.b_field_tor_vacuum_r.data"]
    ...
    for k in ods1['pf_active.coil']:
        identifier = ods1[f'pf_active.coil.{k}.identifier'].upper().replace('PEF','EF')
        if 'WALL' in identifier:
            sigma = 0.20
        else:
            sigma = 0.001
        ...
        try:
            ...
            ods[f'pf_active.coil.{k}.current.data_error_upper'] = sigma*abs(ods[f'pf_active.coil.{k}.current.data'])
            if verbose: print(identifier+" loaded")
        except:
...
def pegasus_pf_hardware(ods, user_argument='initial Pegasus III model'):
    ...
    fc_dat = np.array(
        [[0.1038085, 0.89506501, 0.0161, 0.021468, 0.0, 0.0, 0, 1.0, 'DIV1A'], #DIV1A - 10 elements
         ...
         [0.4406265, 1.067, 0.004953, 0.08255, 0.0, 0.0, 8, 1.0, 'pEF1A'], #EF1A - 5 elements
         [0.4458589, 1.067, 0.004953, 0.08255, 0.0, 0.0, 8, 1.0, 'pEF1A'],
         [0.4510913, 1.067, 0.004953, 0.08255, 0.0, 0.0, 8, 1.0, 'pEF1A'],
         [0.4563237, 1.067, 0.004953, 0.08255, 0.0, 0.0, 8, 1.0, 'pEF1A'],
         [0.4615561, 1.067, 0.004953, 0.08255, 0.0, 0.0, 8, 1.0, 'pEF1A'],        
         [0.4731385, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'], #EF1B - 9 elements
         [0.4783709, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.4836033, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.4888357, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.4940681, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.4993005, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.5045329, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.5097654, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],
         [0.5149978, 1.067, 0.004953, 0.08255, 0.0, 0.0, 9, 1.0, 'pEF1B'],        
         [0.90007, 1.0807, 0.01905, 0.01905, 0.0, 0.0, 10, 1.0, 'pEF2'], #EF2 - 3 elements
         [0.90007, 1.0599999, 0.01905, 0.01905, 0.0, 0.0, 10, 1.0, 'pEF2'],
         [0.90007, 1.03943, 0.01905, 0.01905, 0.0, 0.0, 10, 1.0, 'pEF2'],       
         [1.14823, 0.77582997, 0.01905, 0.01905, 0.0, 0.0, 11, 1.0, 'pEF3'], #EF3 - 5 elements
         [1.14823, 0.75481999, 0.01905, 0.01905, 0.0, 0.0, 11, 1.0, 'pEF3'],
         [1.14823, 0.73382002, 0.01905, 0.01905, 0.0, 0.0, 11, 1.0, 'pEF3'],
         [1.14823, 0.71280998, 0.01905, 0.01905, 0.0, 0.0, 11, 1.0, 'pEF3'],
         [1.14823, 0.69181001, 0.01905, 0.01905, 0.0, 0.0, 11, 1.0, 'pEF3'],   
         [1.14823, 0.35124001, 0.01905, 0.01905, 0.0, 0.0, 12, 1.0, 'pEF4'], #EF4 - 5 elements
         [1.14823, 0.33059999, 0.01905, 0.01905, 0.0, 0.0, 12, 1.0, 'pEF4'],
         [1.14823, 0.30996001, 0.01905, 0.01905, 0.0, 0.0, 12, 1.0, 'pEF4'],
         [1.14823, 0.28992999, 0.01905, 0.01905, 0.0, 0.0, 12, 1.0, 'pEF4'],
         [1.14823, 0.26868999, 0.01905, 0.01905, 0.0, 0.0, 12, 1.0, 'pEF4'],      
         [1.14823, -0.35123, 0.01905, 0.01905, 0.0, 0.0, 13, 1.0, 'pEF5'], #EF5 - 5 elements
         [1.14823, -0.33059001, 0.01905, 0.01905, 0.0, 0.0, 13, 1.0, 'pEF5'],
         [1.14823, -0.30996001, 0.01905, 0.01905, 0.0, 0.0, 13, 1.0, 'pEF5'],
         [1.14823, -0.28992, 0.01905, 0.01905, 0.0, 0.0, 13, 1.0, 'pEF5'],
         [1.14823, -0.26868001, 0.01905, 0.01905, 0.0, 0.0, 13, 1.0, 'pEF5'],       
         [1.14823, -0.77582997, 0.01905, 0.01905, 0.0, 0.0, 14, 1.0, 'pEF6'], #EF6 - 5 elements
         [1.14823, -0.75481999, 0.01905, 0.01905, 0.0, 0.0, 14, 1.0, 'pEF6'],
         [1.14823, -0.73382002, 0.01905, 0.01905, 0.0, 0.0, 14, 1.0, 'pEF6'],
         [1.14823, -0.71280998, 0.01905, 0.01905, 0.0, 0.0, 14, 1.0, 'pEF6'],
         [1.14823, -0.69181001, 0.01905, 0.01905, 0.0, 0.0, 14, 1.0, 'pEF6'],        
         [0.90007, -1.0807, 0.01905, 0.01905, 0.0, 0.0, 15, 1.0, 'pEF7'], #EF7 - 3 elements
         [0.90007, -1.0599999, 0.01905, 0.01905, 0.0, 0.0, 15, 1.0, 'pEF7'],
         [0.90007, -1.03943, 0.01905, 0.01905, 0.0, 0.0, 15, 1.0, 'pEF7'],        
    ...
    for coil in fc_dat:
        ...
        else: #oblique coil geometry - converting to IMAS outline format
            alpha = float(coil[4])*np.pi/180
            beta = (float(coil[5])-90.0)*np.pi/180
            dr1 = float(coil[2])*np.cos(alpha)/2    # dr_alpha
            dz1 = float(coil[2])*np.sin(alpha)/2    # dz_alpha

            dr2 = float(coil[3])*np.cos(beta)/2     # dr_beta
            dz2 = float(coil[3])*np.sin(beta)/2     # dz_beta

            R = np.array([-dr1+dr2, dr1+dr2, dr1-dr2, -dr1-dr2])+float(coil[0])
            Z = np.array([-dz1+dz2, dz1+dz2, dz1-dz2, -dz1-dz2])+float(coil[1])

            ods['pf_active']['coil'][coil[6]]['element'][count]['geometry.geometry_type'] = 1
            ods['pf_active']['coil'][coil[6]]['element'][count]['geometry.outline.r'] = R
            ods['pf_active']['coil'][coil[6]]['element'][count]['geometry.outline.z'] = Z
...
def pegasus_limiter(ods, user_argument='initial Pegasus III model'):
    ...
    ods['wall.description_2d.:.limiter.unit.0.outline.r'] = limiter[:,0]
    ods['wall.description_2d.:.limiter.unit.0.outline.z'] = limiter[:,1]
...
def magnetics_hardware(ods):
    ...
    silop = [[0.0606, 0.696, 'CFL01'],
             ...
             [0.06058, -0.3508, 'CFL10'],   # KNOWN BAD SIGNAL
             [0.0606, -0.5615, 'CFL11'],
             [0.0606, -0.6305, 'CFL12'],    # KNOWN BAD SIGNAL
             [0.0606, -0.6995, 'CFL13'],
    ...
    mprobes = [[0.06755, 0.6945, 0.0, pi/2, 'CPA01'],
               ...
               [0.06755, -0.492, 0.0, pi/2, 'CPA18'],   # KNOWN BAD SIGNAL
