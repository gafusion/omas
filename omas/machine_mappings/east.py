...
def setup_langmuir_probes_hardware_description_east(ods, pulse=85282):
    ...
    try:
        ...
        printd('No limiter in ods. Gathering it from MDSplus to support ', 'east')
    ...
    for ul in ['upper', 'lower']:
        for oi in ['outer', 'inner']:
            ...
            try:
                ...
                printd('Failed MDSplus data check for {}; data invalid. Halting.'.format(pointname), 'east')
            else:
                # Data appear to be valid; proceed
                printd('Processing data for probe group {}; {}'.format(corner.upper(), pointname), 'east')
