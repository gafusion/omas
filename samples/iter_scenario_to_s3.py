...
lines=subprocess.Popen('scenario_summary -c machine,shot,run,ref_name,workflow,idslist',stdout=subprocess.PIPE, shell=True).communicate()[0].split('\n')
scenarios=[]
ksep=0
for line in lines:
    if line.startswith('----'):
        ...
        scenarios.append({'machine':items[0],'shot':int(items[1]),'run':int(items[2]),'ref_name':items[3],'workflow':items[4],'idslist':items[5:]})

# setup environmental variables
tmp=subprocess.Popen('imasdb /work/imas/shared/iterdb/3 ; env | grep MDSPLUS_TREE_BASE',stdout=subprocess.PIPE, shell=True).communicate()[0].split('\n')
for line in tmp:
