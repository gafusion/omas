...
all_install_require = ['uncertainties', 'pint', 'netCDF4', 'boto3', 'matplotlib', 'scipy', 'h5py', 'pymongo', 'dnspython','xmltodict', 'pyro5']
install_requires = {}
...
if os.path.exists(here + '.git') and not os.path.exists(here + 'requirements.txt'):
    for version in [2, 3]:
        with open(here + 'requirements_python%d.txt' % version, 'w') as f:
            ...
            for item in sorted(install_requires[version], key=lambda x:x.lower()):
